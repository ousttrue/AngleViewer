...
const auto CONFIG_FILE = L"AngleViewer.json";
...
///
static std::string read_allbytes(const std::wstring &path)
{
...
{
    static void to_json(json &j, const Configuration &data)
    {
        j = json{
            {"window", data.window},
        };
    ...
    static void from_json(const json &j, Configuration &data)
    {
...
static void RestoreWindowPlacement(HWND hWnd, UINT showCmd)
{
    auto config_str = read_allbytes(CONFIG_FILE);
    if (config_str.empty())
    {
    ...
    int nCmdShow)
{
    ...
    RestoreWindowPlacement(hwnd, SW_SHOWNORMAL);
